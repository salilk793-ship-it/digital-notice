  ...
  try {
    ...
    if (notice.publishedById !== user.id) {
      return NextResponse.json({ 
        error: 'You can only permanently delete notices that you created' 
      }, { status: 403 });
    }

    // Delete related records first
    await db.viewEvent.deleteMany({ where: { noticeId: id } });
    await db.reaction.deleteMany({ where: { noticeId: id } });
